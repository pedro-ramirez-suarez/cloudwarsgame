...
Produced by CodeLead `/modernize --plan` on 2026-09-23 from a read-only survey of this repository. Nothing outside this folder was written.

## Contents

- `DOSSIER.md` and `dossier-index.json`: the survey (sections D1–D14, each labelled automated / partial / manual) with its per-section artifacts.
...
- `intake.json` and `intake.md`: the client's goals and constraints (answered in the session); rows marked assumed took a default.
