# Legacy System Dossier — cloudwarsgame

Generated 2026-09-21T20:44:09.898Z · CodeLead Dossier assembler v0

## Coverage summary

**61% tool-driven by section weight** — 5 automated, 7 partial, 2 manual.
...
|---|---|---|---|---|
| D1 | System inventory & architecture map | **AUTOMATED** | `project-map.{json,md}` | 12 modules, 150 files, 24 module-level dependencies (Mermaid diagram included). |
| D2 | Contracts & integrations catalogue | **PARTIAL** | `contracts.{json,md}` | 65 public types and 186 public methods across 9 areas; 1 external integration points (config values redacted). Consumer mapping ("who breaks") is manual in v0. |
...
| D4 | History intelligence | **AUTOMATED** | `git-history.{json,md}` | 41 commits by 3 authors; 25 hot files, 22 co-change pairs, 3 knowledge-concentration rows. |
...
| D10 | Browsable knowledge base | **PARTIAL** | `..\..\..\src\cloudwarsgame\docs\codelead\dossier\.codelead\knowledge.json` | Machine-derived findings seeded as trust-labeled knowledge entries (below); the PKB instance renders them. Interview-sourced entries are added during the engagement. |

## Knowledge base seeded (D10)

44 findings recorded into `C:\src\cloudwarsgame\docs\codelead\dossier\.codelead\knowledge.json` from the scans above; the store deduplicates by (type, summary), so it holds fewer distinct entries than that —
